...
- **Dampak agregat.** Efek positif +4,85 pp tenggelam oleh tandan yang gagal
  ...
  sementara galat pencacahan (*counting*) membengkak menjadi MAE **3,34**, dibandingkan
  Garis dasar pembanding (*baseline*) Ridge **1,05**.

## 4. Solusi yang Belum Dicoba untuk Mengejar Target $\geq 80\%$

- **Modul C2/C3, pengklasifikasi terpisah.** Mengganti skor detektor YOLO (C1)
  dengan *backbone* khusus seperti ConvNeXt atau Transformer Multi-View untuk
  menilai potongan tandan.
- **fungsi rugi ordinal.** Mengganti Cross-Entropy biasa dengan Ordinal Regression
  Fungsi rugi agar galat antarkelas bersebelahan, terutama B2 versus B3, makin kecil.
...
cakupan penaut dan pengklasifikasi khusus C2/C3 merupakan jalur untuk melampaui
...
(deteksi, klasifikasi per-tampak/per-tandan, penautan, dan pencacahan) boleh
...
telah mempersempit hambatan struktural (*bottleneck*) secara lebih tepat:

- propagasi skor keyakinan (*confidence*) lintas-view yang dikunci di validation menaikkan kepala
...
- setelah kelas dilipat, proposal fisik yang sama mencapai AP50 **0,8381** dan
  ...
  pengungkit terbesar;
- relabel probabilistik berbasis pengklasifikasi citra terpotong (*crop classifier*) menaikkan keempat AP50 kelas,
...
- pengklasifikasi strict terbaik pada kotak dan tautan GT masih ConvNeXt residual:
...
- pencacahan macro terbaik tetap regresor multi-ambang dengan macro-MAE
  **1,0039**. Full multi-bank penyesuaian berlebih (VAL 0,8110, TEST 1,0374). Audit memisahkan
...
ordinal untuk kelas tandan, dan regresi multi-bank untuk pencacahan. RF-DETR-L,
...
Satu pengungkit kelas tambahan kini punya dasar data yang kuat: pengklasifikasi
lama belajar dari citra terpotong (*crop*) kotak GT, padahal saat deploy ia menerima kotak prediksi.
