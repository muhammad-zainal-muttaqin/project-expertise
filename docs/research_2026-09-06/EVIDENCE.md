...
- Guo dkk., ICML 2017, [On Calibration of Modern Neural Networks](https://proceedings.mlr.press/v70/guo17a.html): skor keyakinan (*confidence*) perlu diuji kalibrasinya; kalibrasi sendiri tidak memulihkan objek atau menjamin generalisasi.
...
| A20 | Sampel re-ID tidak didominasi jumlah negatif dari pohon sama sebagaimana komentar | `reid_pertandan.py` | Pada 16 pohon ×20 citra terpotong (*crop*), hanya 18 dari 318 negatif berasal dari pohon sama. Tidak menyimpulkan dominasi gradien tanpa pengukuran. |
...
| A24 | Citra terpotong Fase 6 BGR dengan normalisasi RGB ImageNet | `build_crop_dataset.py`, `train_crop_classifier.py`, probe | Terbukti; train/infer sama-sama BGR. Tidak berlaku pada Panen berbasis PIL RGB. |
| A25 | AF-E-005 menganggap hasil satu pengklasifikasi sebagai plafon dataset | `exp_ceiling.py`, `exp_ceiling.log` | 0,6569 bersyarat pada P_test pengklasifikasi; bukan batas Bayes/kualitas anotasi. Simulasi akurasi 0,90 juga bersyarat pada resep galat/skor buatan. |
...
| A33 | Distribusi CORN direduksi menjadi rerata ordinal | `panen_pipeline.py:131`, `corn_cached_diagnostic` | Kehilangan informasi secara matematis. Pada cache citra terpotong saat ini aturan kasar berbeda hanya 2/2.612 test, tanpa perbaikan akurasi bersih; jangan menjanjikan perbaikan besar. |
