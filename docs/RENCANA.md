...
**Tujuan:** Pastikan pipeline pencacahan (*counting*) bisa menerima output dari detektor
...
| 0,3 | Verifikasi nilai acuan kebenaran (*ground truth*) | Pastikan anotasi SawitMVC-Depth kompatibel dengan pipeline pencacahan (format JSON, kelas B1–B4, identitas tandan) | **Selesai**, skema identik dengan SawitMVC (lihat `docs/SCHEMA-PERTREE.md`), tidak perlu shim terjemahan |
...
## Fase 1, RGB 953 Pohon (SawitMVC): Pencacahan

**Tujuan:** Isi sel pencacahan untuk ketiga detektor pada dataset 953 pohon.
Angka deteksi sudah ada dari E-021, yang kurang hanya pencacahan.
...
| 1,4 | Pencacahan ketiga model | Jalankan pipeline Ridge + F_all untuk ketiga set prediksi | Belum |
| 1,5 | Bandingkan | Apakah detektor dengan mAP lebih tinggi juga memberi pencacahan lebih baik? | Belum |
...
**Estimasi waktu:** ~1 jam GPU (inference saja), ~30 menit CPU (pencacahan).

---

## Fase 2: RGB 352 Pohon (SawitMVC-Depth, tanpa depth)

**Tujuan:** Bangun garis dasar pembanding (*baseline*) RGB pada dataset yang sama yang nantinya akan
...
| 2,5 | Inference + pencacahan | Pipeline Ridge + F_all | Belum |
...
(RGB + depth), bandingkan dengan garis dasar pembanding RGB Fase 2.
...
| 3,3 | Modifikasi RF-DETR-L | **Paling sulit.** DINOv2 kerangka utama (*backbone*) beku, stem 3-kanal. Opsi: (a) tambah proyeksi 1-kanal → 3-kanal lalu concat, (b) tulis stem baru 4→embed_dim, (c) fusi menengah dengan cabang terpisah | Belum |
...
| 3,6 | Inference + pencacahan | Pipeline Ridge + F_all | Belum |
...
| Kerangka utama beku menolak kanal ke-4 | Seri F (gate init-nol) | Jangan gunakan gate init-nol; gunakan warmup atau inisialisasi kecil-taknol |
...
| 4,2 | Kompilasi matriks pencacahan | Class &plusmn;1 Acc, Tree &plusmn;1 Acc, MAE, semua 9 sel |
...
tuning hyperparameter (sudah terbukti mencapai batas saturasi, lihat "Hal yang sudah dicoba
...
- Arsitektur: cabang depth terpisah dengan fusi menengah, inisialisasi
  ...
  indikatif E-032 (mid-fusion 3/3 seed positif, CI masih mencakup nilai nol).
...
seperti Fase 1-3), penyaringan awal (*screening*) cepat, bukan angka final. Alasan pengguna:
sinyal menjanjikan atau tidak sudah terlihat di 15 epoch pertama. Konsekuensi:
- Angka dari penyaringan awal 15-epoch **tidak dibandingkan langsung** dengan angka
  60-epoch Fase 1-3, hanya dipakai untuk ranking relatif antar-percobaan
  Fase 5 (mana yang layak dilanjutkan).
- Kandidat yang lolos penyaringan awal (naik konsisten, bukan derau) baru dijalankan
...
- Tetap dicatat di `EKSPERIMEN.md` sebagai entri sendiri (mis. tag
  ...
  berdasar sinyal penyaringan awal, bukan diklaim sebagai hasil final.

### Status pelaksanaan (2026-08-11)

Penyaringan awal kedua lever sudah dijalankan pada YOLO26l (prioritas pertama sesuai
...
- **Lever representasi: 4 kandidat di-screening**: `edge` (Sobel gradient
  magnitude) unggul secara jelas (val mAP50 0,3777 vs 0,3168/0,3221/0,3321 untuk
  ...
  0,3919), robust terhadap garis dasar pembanding RGB manapun. **Counting INCONCLUSIVE**
, bootstrap CI berpasangan berbalik arah tergantung garis dasar pembanding RGB-352 yang
  dipakai (unggul +3,18 pp dibanding retrain baru, lebih rendah −2,28 pp dibanding angka asli
...
- **Lever arsitektur, mid-fusion + gate non-zero-init**: gate berhasil
  ...
  mAP50 0,209), berada jauh di bawah kandidat representasi pada jumlah epoch
  ...
  (kondisional pada lever arsitektur YOLO26l lolos penyaringan awal, yang tidak
  terjadi).

**Fase 5 SELESAI** (2026-08-11), semua metrik terisi (deteksi, pencacahan,
...
| 1 | Pengklasifikasi kematangan pada citra terpotong (*crop*), RGB vs RGB+relief-depth | menyerang headroom 0,2970; citra terpotong melakukan pooling wilayah by-construction |
| 2 | Detektor class-agnostic (1 kelas "tandan"), pretrain 953 → penyesuaian terarah (*fine-tuning*) 352 | lokalisasi lihat 2.299 positif, bukan terpecah jadi 215 (B3) / 98 (B4) |
...
difusikan **setelah global pooling**, gate init taknol (F-007), plus fungsi rugi
auxiliary RGB-only supaya jalur RGB tidak bisa dirusak jalur depth.

### Batas yang wajib disebutkan

Test split 352 hanya 410 box, dengan B4 = 26. Selisih kecil pada mAP50 tidak
