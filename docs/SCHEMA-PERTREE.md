...
- `annotations`: list per bbox. `class_name` &isin; {B1,B2,B3,B4}. `bbox_yolo`
  = [cx, cy, w, h] ternormalisasi (format YOLO standar). `conf` = skor keyakinan (*confidence*)
  deteksi (0-1).
...
pencacahan (*counting*) Fase 1 yang benar mengikuti pola `exp_counting_v3.py`** (fit Ridge
...
## Kompatibilitas nilai acuan kebenaran SawitMVC vs SawitMVC-Depth (verifikasi Fase 0.5)
