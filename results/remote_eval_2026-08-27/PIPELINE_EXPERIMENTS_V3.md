...
Sweep besar sudah mencakup perubahan citra, model citra terpotong (*crop*), fusi prediksi,
penaut empat sisi, dan penyesuaian terarah (*fine-tuning*) detektor. Tidak ada konfigurasi yang
...
|---|---|---:|---|
| Lokalisasi agnostik AP50 | SawitMVC-Depth | **87,64%** | WBF tiga detektor |
| Lokalisasi agnostik AP50 | SawitMVC-YOLO 953 | **83,50%** | WBF tiga detektor |
...
| Akurasi kelas, profil class-priority | SawitMVC-YOLO 953 | **71,81%** | precision/pencacahan (*counting*) turun |
| F1 fisik pipeline | SawitMVC-YOLO 953 | **80,43%** | profil standar |
| MAE pencacahan | SawitMVC-YOLO 953 | **1,393** | tandan per pohon |

Angka 83,50% adalah AP50 lokalisasi class-agnostic. Itu bukan akurasi empat
kelas, bukan F1 pencacahan, dan bukan bukti bahwa seluruh pipeline mencapai 83%.
...
|---|---|---|
| Photometric | hue/color MLP, CLAHE, sharpening, gamma 0,95/1,05, brightness/contrast | ditolak; tidak stabil atau tidak mengungguli garis dasar pembanding (*baseline*) |
| Color correction | gray-world dan mild white-balance | ditolak; E2E terbaik hanya 71,62% dan 71,88% pada validation |
| Citra terpotong head | RGB ConvNeXt Tiny 224/256, ConvNeXt Small 224, Swin Tiny | Tiny/Small paling kompetitif; tidak mencapai 75% pada test |
| Training objective | class weighting, focal penyesuaian terarah, ordinal BCE, GT-pretrain | proposal head membaik sedikit, E2E tidak |
...
| Detektor | YOLO/RT-DETR local penyesuaian terarah dan YOLO resolusi 1600 | validation lebih rendah atau terlalu mahal tanpa gain |
| Postprocess | WBF threshold/weight, class profile, class-confidence rank, support rank | kompromi performa (*trade-off*) precision vs class accuracy; tidak semua target tercapai |
| Pencacahan | count blending dan cluster-quality reranking | pencacahan/kelas tidak membaik secara simultan |
...
| Small + profil all-round + count blend | 72,62% | **81,51%** | 1,330 | **55,98%** | kompromi performa terbaik, masih <75% |
...
| Focal penyesuaian terarah | 71,88% | 80,87% | 1,253 | — | ditolak |
...
### Profil class-priority: hanya sebagai kompromi performa
...
2. Kesalahan kelas bersifat ordinal: B2 sering tertukar dengan B3, dan B3
   ...
   sharpening global.
3. Head citra terpotong memang belajar sinyal tambahan, proposal validation terbaik sekitar
...
4. Validation class-priority 74,06% memiliki precision/F1 fisik yang lebih
   rendah dan turun pada test. Ini indikasi penyesuaian berlebih (*overfitting*) pada profil validasi,
   bukan bukti kemampuan general.
5. Model batas atas teoretis (*oracle*) disagreement antara garis dasar pembanding dan head menunjukkan masih ada ruang
   teoritis, tetapi gate yang diuji (logistic, ExtraTrees, margin, support,
   skor keyakinan (*confidence*), dan cluster quality) tidak dapat menemukan pemilih yang
   general.
...
- Untuk empat kelas pada test 953, performa realistis saat ini sekitar 71,7%
  akurasi kelas pada match, dengan F1 fisik sekitar 80,4% dan MAE pencacahan
  sekitar 1,39.
...
- penyesuaian terarah detektor class-aware dengan data tambahan/oversampling B4,
  bukan hanya postprocess proposal;
- menambah capture empat sisi yang baru untuk validasi eksternal;
...
menambah layer warna, sharpening, atau reranker hanya menggeser kompromi performa dan
...
- [`README.md`](README.md), garis dasar pembanding dan protokol awal;
...
- [`test_final_classpriority.json`](/workspace/model_artifacts/project-expertise/test_final_classpriority.json), kompromi performa class-priority;
