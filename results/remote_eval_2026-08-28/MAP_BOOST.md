...
**Selang kepercayaan *bootstrap*:** tidak dihitung — lihat Batasan Validitas & Audit (poin 4).
...
3. **Hasil dilaporkan apa adanya, termasuk regresi.** Tiga dari empat metrik menunjukkan peningkatan performa kecil dan konsisten arah dengan VAL (953 agnostik +0,0069; 953 sadar-kelas +0,0109; *depth* agnostik +0,0019). **Profil sadar-kelas *depth* justru mengalami penurunan performa pada test (Δ=−0,0139, dari baseline 0,6691 menjadi 0,6552)**, berlawanan arah dengan keunggulan yang terlihat di VAL (0,6623 vs. baseline VAL 0,6595, yaitu +0,0028). Karena bootstrap CI tidak dihitung (lihat Batasan Validitas & Audit), tidak dapat disimpulkan apakah selisih +0,0028 di VAL tersebut memang lebih besar daripada derau sampel VAL sejak awal — kemungkinan profil ($a$=1; $b$=0; artinya $p_{tp}$ dari *re-ranker* sama sekali tidak dipakai untuk sadar-kelas *depth*, murni skor WBF × $p_c$) terpilih pada titik yang secara kebetulan menguntungkan pada populasi VAL, dan konsisten dengan menyempit atau berbalik pada test dengan derau sampel yang sebanding. Keputusan pada sesi ini adalah **mempertahankan profil sadar-kelas *depth* apa adanya** (bukan memilih ulang berdasarkan angka test — hal ini dilarang oleh disiplin *single-look*), sambil menandai regresi ini secara eksplisit sebagai temuan generalisasi negatif yang perlu diinvestigasi pada sesi berikutnya (kandidat: uji apakah menyertakan $p_{tp}$ ($b>0$) pada sadar-kelas *depth* menstabilkan performa lintas *split*, memakai bootstrap CI berpasangan VAL vs. test untuk mengonfirmasi apakah selisihnya signifikan).
4. **Tidak ada CI *bootstrap* untuk keempat metrik test-locked** karena kedua skrip `bootstrap_map*.py` yang tersedia tidak kompatibel tanpa modifikasi (rincian di Batasan Validitas & Audit, poin 4); sesuai instruksi tugas, tidak dibangun mesin *bootstrap* baru untuk menutup celah ini pada sesi ini.
...
3. **Bootstrap CI dilewati; delta test kecil tidak memiliki ukuran ketidakpastian.** `bootstrap_map.py` meng-*hardcode* dataset `/workspace/SawitMVC-Depth` (split kanonik 352, pemuat GT berbasis *file* teks YOLO sendiri, ukuran citra tetap 1280×800) tanpa parameter untuk menunjuk ke `data_root` `SawitMVC-YOLO`/`SawitMVC-Depth-YOLO` yang dipakai pipeline ini — tidak kompatibel tanpa modifikasi. `bootstrap_map_from_npz.py` meng-*hardcode* kamus `CORPORA` (`new763`, `combined1716`) dengan jalur prediksi satu-model tetap dan jalur keluaran tetap, `main()`-nya tidak menerima argumen CLI apa pun, dan fungsi AP50-nya sendiri adalah implementasi ulang buatan tangan (bukan `base.coco_metrics`/pycocotools) — bukan titik masuk generik yang menerima kamus submisi sembarang. Menyambungkan salah satu skrip ke submisi hasil fusi/*re-ranking* pada sesi ini memerlukan kode perekat baru di sekitar internalnya, yang dilarang secara eksplisit oleh instruksi tugas ("jangan membangun mesin *bootstrap* baru"). Akibatnya, keempat delta test pada tabel di atas — termasuk regresi sadar-kelas *depth* sebesar −0,0139 — dilaporkan sebagai titik estimasi tunggal tanpa selang kepercayaan; besar/kecilnya delta relatif terhadap derau pengambilan sampel citra test **tidak diketahui** pada sesi ini.
