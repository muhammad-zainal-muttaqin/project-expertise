...
The mAP skor keyakinan (*confidence*) intervals use 500 paired image-level resamples.  The
end-to-end intervals use 5,000 paired tree-level resamples over saved
per-tree summaries; no image, detektor, or model inference is performed by
that analysis.

### Detektor mAP track

| Test set | Garis dasar pembanding (*baseline*) | Locked result | Delta | Paired 95% CI for delta | Supported? |
|---|---:|---:|---:|---:|---|
...
`ci_boot/artifacts/CI_SUMMARY.md`.  The Depth class-aware garis dasar pembanding remains
the production profile.

### End-to-end track

| Dataset | Metric | Garis dasar pembanding | Locked result | Improvement (positive is better) | Paired 95% CI | Supported? |
|---|---|---:|---:|---:|---:|---|
...
`gsp_artifacts/depth/results_test_locked.json`, and the original garis dasar pembanding
...
macro-F1 intervals because the legacy garis dasar pembanding stores only aggregate class
...
| Dataset | Frozen VAL garis dasar pembanding matched / macro | Candidate | Matched | Macro-F1 | Physical F1 | MAE | ±1 |
|---|---:|---|---:|---:|---:|---:|---:|
...
DINOv2-Large branch was run only on TRAIN/VAL citra terpotong (*crop*) and uses 2,048-dimensional
...
detektor anchor and promote a branch only when it improves the declared
...
* an adaptive Hungarian-versus-GSP policy.  Its TRAIN utility model batas atas teoretis (*oracle*) is only
...
* rich graph/count features and nonlinear count regressors.  Although their
  ...
  VAL physical/count kompromi performa (*trade-off*) worsened, so the original count layer remains
  the reference.

These negative results are retained as explicit ablations.  They prevent a
larger model, an apparently lower count-regression error, or a TRAIN model batas atas teoretis
...
Large citra terpotong arrays and feature matrices remain outside the repository tree;
...
the GSP recall kerangka utama (*backbone*), borrow the better count target, and let the class
...
A separate head-aware truncation studi ablasi (*ablation study*) showed why the linker score remains
the primary ranking signal. Member-head skor keyakinan increased matched class
accuracy to `0,7718` on 953 and `0,8603` on Depth, but reduced physical F1 by
`0,0064` and `0,0037`, respectively. It is retained as a negative studi ablasi;
