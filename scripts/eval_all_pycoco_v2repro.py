"""Eval pycocotools tiga model v2repro (YOLO26l, RT-DETR-L, RF-DETR-L) pada 953 pohon.

Protokol identik dengan eval_all_pycoco.py Volume 1, tapi arah ke bobot _v2repro.
GT dibangun dari label YOLO SawitMVC-YOLO, evaluasi via COCOeval.
"""
import argparse
import json
...
MODELS_953 = [
    ("YOLO26l",   "yolo",   "models/yolo26l_e60_i1280_v2repro/best.pt",                   1280, 26.3),
    ("RT-DETR-L", "rtdetr", "runs/rtdetr_l_e60_i1280_v2repro/weights/best.pt",             1280, 33.0),
    ("RF-DETR-L", "rfdetr", "runs/rfdetr_l_e60_i1280_v2repro/checkpoint_best_ema.pth",     1280, 35.7),
]


def build_gt(ds_root, split_dir):
    images, anns, ann_id = [], [], 1
    idir = ds_root / "images" / split_dir
    ldir = ds_root / "labels" / split_dir
...
def main():
    ...
    parser.add_argument("--dataset-root", default="/workspace/SawitMVC-YOLO")
    parser.add_argument("--splits", nargs="+", default=["val", "test"])
    args = parser.parse_args()

    proj = Path(args.project_root)
    ds = Path(args.dataset_root)
    out = proj / "results" / "perkelas_pycoco_v2repro.json"

    split_map = {"val": "val", "test": "test"}
    gts = {s: build_gt(ds, split_map[s]) for s in args.splits}

    data = json.loads(out.read_text()) if out.exists() else {}
    for key, kind, rel_weights, imgsz, params in MODELS_953:
