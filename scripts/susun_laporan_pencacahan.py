...
def bagian_deteksi(s: Sumber) -> list[str]:
    L = ["## 4. Deteksi per Korpus Latih", "",
         ...
         "Presisi, *recall*, dan F1 diukur pada ambang $" + B + "text{conf}^{*}$. Tebal: nilai tertinggi tiap korpus.", "",
...
def bagian_pencacahan(s: Sumber) -> list[str]:
    L = ["## 5. Pencacahan per Korpus Latih", "",
         ...
         "|---|---|---|---|---|---|---|---|"]
    for korpus in KOR:
        baris = {det: s.terbaik(korpus, det) for det in DET}
        det_min = min(DET, key=lambda d: baris[d]["metrik"]["mae_makro"])
        for det in DET:
            b = baris[det]
            m = b["metrik"]
            t = det == det_min
...
def bagian_efek(s: Sumber) -> list[str]:
    ...
    L += ["", "Tebal: $MAE$ terendah tiap pasangan korpus dan detektor.", ""]
    return L


# Kolom perbandingan: (nama, sumber, kunci, arah). Arah +1 berarti makin besar makin baik.
...
def sel_terbaik(grup: list[tuple[dict, dict]]) -> list[list[str]]:
    ...
    for kunci, sumber, arah in KOLOM_BANDING:
        nilai = [ambil(d, m, sumber, kunci) for d, m in grup]
...
def bagian_silang_detektor(s: Sumber) -> list[str]:
    L = ["## 8. Uji Silang Detektor", "",
         ...
         "Detektor diuji pada partisi uji korpus lain, dan koefisiennya dipasang ulang pada partisi sasaran. "
...
def bagian_partisi(s: Sumber) -> list[str]:
    ...
    L += ["", "| Pasangan partisi | Pohon beririsan | Konsekuensi |", "|---|---|---|",
          ...
          f"| Uji `1716` dan uji `763` | {i['uji1716_uji763']} dari {i['n763']} pohon | Basis baris `1716` ke `763` |",
          f"| Uji `1716` dan latih/validasi `763` | {i['uji1716_lihat763']} pohon DEPTH, citra identik | "
          "Dikeluarkan dari baris `763` ke `1716` |",
