"""Counting evaluation untuk tiga detektor RGBD 4-kanal pada 352 pohon.

Langkah:
  1. Inference per-tree pada citra 4-kanal (TIFF) dari SawitMVC-Depth-4ch
  2. Ridge + F_all counting (train+val → test)

Usage:
    python run_counting_rgbd352.py \
        --project-root /workspace/project-expertise \
        --image-dir /workspace/SawitMVC-Depth-4ch/images \
        --gt-dir /workspace/SawitMVC-Depth/json \
        --yolo-dir /workspace/SawitMVC-Depth-4ch-YOLO \
        --rfdetr-dataset /workspace/rfdetr_ds_352_4ch
"""
...
MODELS = [
    ("YOLO26l-RGBD",   "yolo",   "runs/yolo26l_e60_i1280_rgbd352/weights/best.pt",          1280),
    ("RT-DETR-L-RGBD", "rtdetr", "runs/rtdetr_l_e60_i1280_rgbd352/weights/best.pt",         1280),
    ("RF-DETR-L-RGBD", "rfdetr", "runs/rfdetr_l_e60_i1280_rgbd352/checkpoint_best_ema.pth", 1280),
    # Fase 5 (V2-E-010): encoding depth 'edge' menang screening V2-E-008.
    ("YOLO26l-RGBD-edge", "yolo", "runs/yolo26l_e60_i1280_rgbd352_edge/weights/best.pt",    1280),
]
...
def group_images_by_tree(image_dir: Path) -> dict[str, list[Path]]:
    ...
    for ext in ("*.tiff", "*.tif", "*.jpg", "*.png"):
        for img in sorted(image_dir.glob(ext)):
...
def infer_yolo(model, img_paths: list[Path], imgsz: int) -> list[list[dict]]:
    sources = []
    for p in img_paths:
        if p.suffix.lower() in ('.tiff', '.tif'):
            sources.append(cv2.imread(str(p), cv2.IMREAD_UNCHANGED))
        else:
            sources.append(str(p))
    result = model.predict(source=sources, imgsz=imgsz,
...
def infer_rfdetr(model, img_paths: list[Path]) -> list[list[dict]]:
    all_anns = []
    for p in img_paths:
...
def setup_rfdetr_4ch(weights, imgsz, dataset_dir):
    import torch
    sys.path.insert(0, str(Path(__file__).parent))
    from train_rfdetr_4ch import (patch_a_pemuat_tiff, patch_b_normalisasi,
                                   patch_c0_validasi_kanal, statistik_depth_train)
    patch_a_pemuat_tiff()
    patch_c0_validasi_kanal()
    mean_d, std_d = statistik_depth_train(Path(dataset_dir))
    patch_b_normalisasi(mean_d, std_d)
    from rfdetr import RFDETRLarge
    model = RFDETRLarge(num_channels=4, num_classes=4, resolution=imgsz)
    ckpt = torch.load(weights, map_location="cpu")
    model.model.model.load_state_dict(ckpt["model"], strict=False)
    return model


def run_inference(kind: str, weights: str, imgsz: int,
                  trees: dict[str, list[Path]], tree_splits: dict[str, str],
                  out_dir: Path, rfdetr_dataset: str = None) -> int:
    import torch
    if kind == "yolo":
        ...
        model = setup_rfdetr_4ch(weights, imgsz, rfdetr_dataset)
...
def main():
    ...
    ap.add_argument("--image-dir", default="/workspace/SawitMVC-Depth-4ch/images")
    ap.add_argument("--gt-dir", default="/workspace/SawitMVC-Depth/json")
    ap.add_argument("--yolo-dir", default="/workspace/SawitMVC-Depth-4ch-YOLO")
    ap.add_argument("--rfdetr-dataset", default="/workspace/rfdetr_ds_352_4ch")
    args = ap.parse_args()
    ...
    print(f"Trees: {len(trees)}, GT: {len(gt_map)}, Splits: "
          f"train={sum(1 for v in tree_splits.values() if v=='train')} "
          f"val={sum(1 for v in tree_splits.values() if v=='val')} "
          f"test={sum(1 for v in tree_splits.values() if v=='test')}")

    out_file = proj / "results" / "counting_rgbd352.json"
    ...
    for label, kind, rel_weights, imgsz in MODELS:
        ...
        pertree_dir = proj / "runs" / "pertree_rgbd352" / f"{kind}_{label.lower().replace('-','')}"
        print(f"\n===== {label} ({kind}) =====")
        print(f"  Inference on {len(trees)} trees...")
        n = run_inference(kind, weights, imgsz, trees, tree_splits, pertree_dir, args.rfdetr_dataset)
        ...
        m = score(y_te, model_ridge.predict(X_te))
