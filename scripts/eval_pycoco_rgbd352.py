"""Eval pycocotools tiga model RGBD 4-kanal pada 352 pohon.

Usage:
    python eval_pycoco_rgbd352.py --project-root /workspace/project-expertise \
        --dataset-root /workspace/SawitMVC-Depth-4ch-YOLO --splits val test
"""
import argparse
import json
...
def build_gt(ds_root, split_dir):
    ...
    paths = sorted(p for p in idir.iterdir()
...
def predict_ultra(model, paths, imgsz):
    ...
    for i, p in enumerate(paths, 1):
        if p.suffix.lower() in ('.tiff', '.tif'):
            img = cv2.imread(str(p), cv2.IMREAD_UNCHANGED)
            r = model.predict(source=img, imgsz=imgsz, conf=0.001, verbose=False)[0]
        else:
...
def setup_rfdetr_4ch(weights, imgsz, dataset_dir):
    """Load RF-DETR-L with 4-channel patches for inference."""
    import sys, torch
    sys.path.insert(0, str(Path(__file__).parent))
    from train_rfdetr_4ch import patch_a_pemuat_tiff, patch_b_normalisasi, patch_c0_validasi_kanal, statistik_depth_train
...
def load_model(kind, weights, imgsz, dataset_dir=None):
    if kind == "yolo":
    ...
    return setup_rfdetr_4ch(weights, imgsz, dataset_dir)


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--project-root", default="/workspace/project-expertise")
    parser.add_argument("--dataset-root", default="/workspace/SawitMVC-Depth-4ch-YOLO")
    parser.add_argument("--rfdetr-dataset", default="/workspace/rfdetr_ds_352_4ch")
    parser.add_argument("--splits", nargs="+", default=["val", "test"])
    args = parser.parse_args()

    proj = Path(args.project_root)
    ds = Path(args.dataset_root)
    out = proj / "results" / "perkelas_pycoco_rgbd352.json"

    gts = {s: build_gt(ds, s) for s in args.splits}

    data = json.loads(out.read_text()) if out.exists() else {}
    for key, kind, rel_weights, imgsz, params in MODELS_RGBD:
        weights = str(proj / rel_weights)
        if not Path(weights).exists():
            print(f"SKIP {key}: bobot belum ada ({weights})")
            continue
        print(f"\n===== {key} ({params}jt, imgsz {imgsz}, {kind}) =====")
        model = load_model(kind, weights, imgsz, args.rfdetr_dataset)
