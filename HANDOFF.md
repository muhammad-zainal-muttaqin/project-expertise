...
ke B1/B2/B3/B4 dengan skor keyakinan (*confidence*) yang dapat diaudit.
...
- Split dilakukan pada level tree, bukan image/citra terpotong (*crop*), agar empat sisi pohon
  yang sama tidak bocor antar split.
- Metrik citra terpotong/head tidak boleh disebut sebagai metrik pipeline end-to-end.
- `matched_class_accuracy`, macro-F1, detektor mAP50, physical F1, dan
  pencacahan (*counting*) accuracy adalah metrik berbeda dan harus dilaporkan bersama
  denominator/jumlah match.
- Tidak boleh memakai model batas atas teoretis (*oracle*), memilih member berdasarkan GT, atau membuang
...
|---|---|---|
| **V1/original** | WBF proposal, prior rotasi, linker awal, pengklasifikasi per tandan, pencacahan/reconciliation | [`PIPELINE_EXPERIMENTS_V3.md`](results/remote_eval_2026-08-27/PIPELINE_EXPERIMENTS_V3.md), [`experiments/EKSPERIMEN.md`](experiments/EKSPERIMEN.md) |
...
Metode melatih pengklasifikasi pasangan pada proposal WBF nyata. Label train
...
signed-rotation residual, area/shape, detektor score, soft-class similarity,
entropy/skor keyakinan, dan rank lokal dalam sisi.
...
photometric TTA, citra terpotong 224/256/320, ConvNeXt/Swin variants, class weighting,
fungsi rugi *focal*, ordinal, dan *mixed*, model-vote WBF, count/reranker, local detektor
penyesuaian terarah (*fine-tuning*), group selector, dan agnostic NMS tidak memberi improvement
...
stage 1+2 (172 per dataset), cross-layer 953 (815 detektor + 176 class
...
> diterjemahkan sebagai refleksi kerja agent, inilah catatan paling terbuka:
> bagian tersulit dari proyek ini bukan menjalankan model, melainkan menahan
> diri agar angka yang tampak unggul tidak dipromosikan sebelum
...
> angka itu hanya kemenangan lokal, kompromi performa (*trade-off*), atau bahkan tidak valid.

### 1. Ringkasan tanpa kosmetik

Proyek ini **tidak gagal total**, tetapi target engineering belum tercapai secara
menyeluruh. Hasil yang kuat adalah:
...
- Depth class-aware justru turun dari `0,6691` menjadi `0,6552`; hasil ini
  tidak disembunyikan dan garis dasar pembanding lama tetap menjadi profil produksi.
- Native RGB+D4 pada `new763` tidak memberikan kenaikan signifikan pada satu
  pun dari tiga detektor. Late fusion memberi kandidat validation yang kuat,
...
pencacahan, dan empat kelas pada domain baru tanpa membayar di metrik lain.

### 2. Kronologi progres dan kegagalan

#### Fase fondasi: garis dasar pembanding dan matriks RGB/RGBD (V2-E-001 s.d. V2-E-007)

Tiga detektor: YOLO26l, RT-DETR-L, dan RF-DETR-L, direproduksi pada 953 dan
...
deteksi, sedangkan RT-DETR-L kadang lebih baik pada pencacahan. Ini memaksa kita
...
|---|---|---|---|
| V2-E-005 | RGB+D `inverse` di stem awal tiga detektor | YOLO naik `0,3606→0,3919`, tetapi RT turun `0,4343→0,3877` dan RF turun `0,4544→0,4186` | **Falsified**: depth tidak konsisten membantu |
| V2-E-006 | Pencacahan Ridge + `F_all` dari output RGBD | Class ±1 berubah YOLO `−1,82` pp, RT `−2,27` pp, RF `0,00` pp; tidak ada CI positif | **Falsified** |
| V2-E-007 | Sintesis 9 sel dataset × arsitektur × modalitas | Gain deteksi kecil tidak berpindah ke pencacahan; B4 sangat sensitif terhadap depth | Early fusion ditolak |

Pelajaran pertama yang menyakitkan: **menaikkan mAP satu model tidak otomatis
menaikkan jumlah tandan yang benar**. Pencacahan bergantung pada konsistensi
...
- `dropout`, `clipped`, dan `valid_mask` berada di bawah `edge` pada penyaringan awal (*screening*)
...
- Pencacahan `edge` tidak mengikuti: Class ±1 malah `87,73%→87,27%`, meskipun
  Tree ±1 dan Macro-MAE membaik. Bootstrap selisih pencacahan memiliki CI yang
  mencakup nilai nol. Jadi `edge` dipertahankan sebagai bukti depth dapat membantu
  localization, bukan sebagai solusi end-to-end.
- Mid-fusion dengan cabang depth terpisah dan gate non-zero-init (`V2-E-009`)
  gagal penyaringan awal: puncak hanya `0,2087`, jauh di bawah kandidat `edge`, lalu
...
- Retrain RGB dan paired bootstrap (`V2-E-011`) memberi selisih pencacahan
...
di probe, tetapi saat dipaksa masuk ke stem resolusi penuh, sinyal itu tersisih
oleh variasi acak (*noise*), distribusi kelas, dan objective detektor. Sinyal ada tidak berarti
...
penaut dan pencacahan murni berbasis pool gagal menjadi solusi produksi.

- **PT-E-001 / G0, model batas atas teoretis multi-view berhasil.** Dengan `_confirmedLinks`
  sebagai tautan model batas atas teoretis, aturan ordinal R4 menaikkan akurasi sekitar `+4,36`
  pp dengan CI positif. Ini membuktikan bahwa multi-view memang punya nilai;
  manfaatnya bukan khayalan. Namun model batas atas teoretis tidak boleh dipakai saat inference.
...
- **PT-E-003 / G2, pipeline tanpa GT gugur.** Varian re-ID + probabilitas
  kelas menghasilkan R4 `0,7124` berbanding model batas atas teoretis `0,7360`; gap `−2,36` pp
...
- **Re-ID awal, leakage dan hafalan identitas.** Embedding memiliki AUC train
  ...
  pulih ke `0,3979`, tetapi tetap belum melewati hambatan struktural (*bottleneck*) association.
...
- **PT-E-029 / PT-E-034, bobot linear memiliki plafon.** Ensemble terbobot
  ...
  test-fit. Model batas atas teoretis memilih anggota yang benar hingga `87,39%`, sehingga sinyal
  headroom ada, tetapi selector yang sah belum dapat menemukannya.
- **PT-E-035, skor keyakinan bukan selector yang cukup.** Korelasi skor keyakinan
  terhadap kebenaran hanya `r=0,1185`; mengambil anggota dengan skor keyakinan
...
- **Classifier PT-E-012/014 dan varian C2, tidak konsisten.** ResNet18 dan
  ConvNeXt-Tiny dengan CE/CORAL tidak mengalahkan garis dasar pembanding C1 secara stabil;
...
- **MOE/view stacker/CatBoost/multibank, tidak menjadi all-rounder.** Beberapa
  ...
  memperbaiki class accuracy, physical association, dan pencacahan. Hasilnya
  dipertahankan sebagai studi ablasi (*ablation study*), bukan dihapus.
...
#### Fase diagnosis sinyal dan pengklasifikasi (V2-E-012 s.d. V2-E-021)
...
2. Class-agnostic localization jauh lebih tinggi daripada class-aware mAP.
   ...
   yang lokasinya sudah benar, bukan semata detektor tidak melihat objek.
3. Depth memiliki relief lokal ordinal yang terukur, tetapi SNR per piksel
   ...
   fusion tidak stabil.
4. Pengklasifikasi citra terpotong ConvNeXt mengalahkan klasifikasi detektor satu tahap pada
   benchmark citra terpotong, tetapi keunggulan citra terpotong/head tidak otomatis bertahan sebagai
   metrik end-to-end.
5. Fitur depth yang ditempel ke RGB pengklasifikasi menghasilkan akurasi yang sama
...
8. Pipeline dua-tahap v1–v4 mencapai titik `mAP50≈0,4500`, tetapi replikasi
   ...
   CI `[−0,0286; +0,0663]`. Jadi klaim "dua tahap pasti mengungguli detektor
   sederhana" tidak terbukti secara statistik pada split kecil itu.
9. Training pengklasifikasi citra terpotong gabungan memberi validation tinggi (`0,6953`) dan
   turun di test (`0,6724`); v4 lebih baik untuk mAP, v3 lebih baik untuk
   pencacahan. Objective memang berkonflik.
...
- Split test Depth-352 hanya memiliki 410 box; lebar CI sekitar `0,1167`.
  Banyak selisih kecil tidak mungkin dibedakan dari variasi acak. Ini membuat
...
|---|---:|---|
| 953 RGB+mono vs RGB | `0,4960` vs `0,5436`, delta `−0,0476`, CI `[−0,0671; −0,0274]` | **Mengalami penurunan signifikan** |
| 352 RGB+mono vs RGB | `0,3943` vs `0,3677`, delta `+0,0266`, CI `[−0,0270; +0,0739]` | Naik point estimate, **tidak signifikan** |
| 352 RGB+mono vs sensor edge | `0,3943` vs `0,4270`, delta `−0,0327`, Selang kepercayaan mencakup nilai nol | Tidak unggul |
| 352 RGB+edge+mono vs RGB+edge | `0,3766` vs `0,4270`, delta `−0,0504`, CI `[−0,1038; −0,0015]` | **Mengalami penurunan signifikan** |

Kurva validation 352 bahkan membalik urutan test. Sel 3 dan sel 6 juga
dihentikan lebih awal setelah arah kurva jelas, sehingga besar penurunan performa tidak
...
monocular-depth **tidak pernah unggul signifikan**, dua kali mengalami penurunan signifikan,
dan menambah kanal kelima di atas sensor malah mengencerkan sinyal.

#### Fase garis dasar pembanding korpus baru dan generalisasi lintas domain (V2-E-034 s.d. V2-E-041)
...
tinggi, tetapi cluster duplikat menghancurkan pencacahan.
...
3. Mengganti soft-vote WBF dengan pengklasifikasi citra terpotong C2 secara penuh menurunkan
...
GSP MILP memperbaiki kelemahan union-find garis dasar pembanding dengan constraint struktural
...
- RT RGB+RGBD4 union-NMS mencapai `0,606368` vs RGB `0,577766`, delta
  `+0,028492`, CI `[+0,009231; +0,047236]` pada 200 resample penyaringan awal.
...
- Run RF-DETR v1 dikeluarkan karena ekspansi stem 3→4 terjadi setelah optimizer
  dibuat; kanal depth tidak dilatih. RF-DETR v2 memperbaiki urutan
  ekspansi sebelum optimizer dan menjadi run yang sah.
- RT-DETR RGBD4 memiliki beberapa `NaN` pada fungsi rugi validasi framework, tetapi
...
2. RF-DETR-L CPU-bound, sedangkan YOLO/RT-DETR lebih GPU-bound. Menyamakan
   asumsi training membuat estimasi waktu dan pemakaian sumber daya keliru.
3. Bootstrap RT 500 resample terlalu mahal untuk penyaringan awal, baru sekitar 50/500
   setelah beberapa menit panjang, sehingga dihentikan dan diganti 200 resample
   sebagai screen. Ini bukan CI final publikasi.
4. GPU sempat 0% saat generate mono-depth karena hambatan struktural sebenarnya I/O
...
5. CPU sempat oversubscribed oleh banyak worker dan agent paralel. Paralelisme
   tanpa pembagian sumber daya yang benar tidak sama dengan throughput lebih
   tinggi.
...
- early fusion RGB+D `inverse` pada tiga detektor;
- pencacahan RGB+D `inverse` dengan Ridge + `F_all`;
- encoding depth `dropout`, `clipped`, dan `valid_mask`;
- mid-fusion depth dengan gate, termasuk patch per-instance yang gagal reload;
- klaim bahwa depth sensor otomatis memperbaiki pencacahan;
- depth sebagai fitur tambahan pada pengklasifikasi RGB ketika kontribusinya
...
- C2 pengklasifikasi citra terpotong sebagai pengganti penuh soft-vote detektor;
- blend pengklasifikasi yang hanya memberi kenaikan nominal tanpa validasi baru;
- class weighting, focal penyesuaian terarah, fungsi rugi ordinal dan *mixed* sebagai jaminan E2E;
...
- detektor penyesuaian terarah lokal dan resolusi 1600 sebagai solusi otomatis;
...
### 5. Apa yang berhasil dan dipertahankan
...
- citra terpotong pooling/classification sebagai pemisahan tugas, walaupun head terbaik
...
Urutan hambatan struktural yang paling didukung bukti adalah:
...
2. **Association lintas-sisi**, recall proposal cukup tinggi, tetapi false
   merge dan duplicate cluster merusak pencacahan. Ini sebab GSP membantu F1,
...
boleh dipercaya, komponen mana yang membantu, dan kegagalan mana
