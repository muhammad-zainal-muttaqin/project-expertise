...
> V1 berarti jalur garis dasar pembanding (*baseline*)/original; V2 berarti jalur learned proposal,
...
1. **Kebenaran acuannya sudah ada di dalam dataset vanilla.** `json/*.json`
   ...
   dilatih **dan** diuji, bukan hanya ditebak.
...
4. **Kaveatnya juga sudah terukur.** Penaut geometri-saja hanya mencapai F1
   pasangan 0,4282 di test. Penaut adalah hambatan struktural (*bottleneck*) sebenarnya, dan itulah
   satu-satunya bagian yang perlu diteliti.
...
pencacahan (*counting*) yang sudah ada.
...
| Konfigurasi | `appearance_count` maksimum yang muncul di 9.823 tandan |
|---|---|
...
| Hasil terdahulu | Yang sebenarnya diukur | Mengapa tidak menggugurkan proposal ini |
|---|---|---|
| **E-007**, penautan geometris **DIPALSUKAN**. Ablasi A/B/C semuanya berada di bawah koreksi global k=1,8905 (macro MAE 1,392 vs 0,356) | Kualitas **counting** dari penautan. Penautnya geometri tangan: kelas + ukuran kotak (A), depth (B), pose DA3 (C) | (a) target di sini **kelas per tandan**, bukan jumlah; (b) E-007 tidak pernah memakai **embedding penampilan terlatih**, varian "hanya penampilan" isinya kelas + ukuran kotak saja, bukan piksel; (c) E-007 tidak pernah melaporkan mutu penautannya sendiri (presisi/recall pasangan), hanya galat pencacahan hilirnya |
...
| **E-016**, `multiview_vote` dengan tautan model batas atas teoretis (*oracle*): per-sisi 67,89% → per-tandan 68,55% (val), naik **+0,66 pp** saja | Satu aturan fusi saja: **argmax dari rerata softmax**, ConvNeXt-tiny, potongan GT | Aturan itu membuang dua hal yang diketahui benar tentang label ini: **ordinalitas** (E-012/SR-009: akurasi ±1 = 99,5%) dan **mutu tampak yang tidak seragam**. §5.4 mengusulkan tangga aturan yang memakai keduanya. Kalau tangga itu pun hanya menghasilkan +0,66 pp, gerbang G0 menggugurkan separuh proposal ini, dan itu memang jawabannya |
...
- **Dari yang bisa di-pool, 85,5% punya TEPAT DUA sisi.** Ini menggugurkan mekanisme voting
...
### 4.5 Hambatan struktural sebenarnya: penaut
...
- **AUC 0,93 tapi F1 0,48.** Sinyal peringkatnya kuat, tetapi prevalensi positif
  hanya 6,62% (10.538 dari 159.198 pasangan lintas-sisi), jadi ambang
...
pencacahan" **berlaku sebagian saja**. Angka 37,74% itu untuk gabungan acak;
...
menjadi garis dasar pembanding PT-E-002**, bukan disaingi dari nol.
...
| Kalau garis dasar pembanding itu | Maka |
|---|---|
...
| di bawah ambang G1 | baru embedding re-ID dikerjakan, dan gainnya diukur sebagai **delta terhadap garis dasar pembanding itu**, bukan terhadap 0,4282 |

> ⚠ **Yang saya butuhkan dari Anda:** lokasi algoritma dedup itu (repo, berkas,
> atau alat anotasinya). Ia tidak ada di `/workspace`, yang ada di sini hanya
> keluarannya (graf tautan di dalam JSON) dan penaut geometris E-007 yang sudah
> dipalsukan. Tanpa kodenya, PT-E-002 terpaksa mulai dari garis dasar pembanding geometri
> 0,4282 yang jauh lebih rendah, dan angka gain-nya akan menyesatkan.

Kalau garis dasar pembanding itu perlu diperkuat, yang belum pernah dicoba di repo ini adalah
...
- **Fungsi rugi**: supervised contrastive / InfoNCE, positif = `bunch_id` sama,
...
- **Skor pasangan**: `s(i,j)` = HistGradientBoosting atas 12 fitur geometri §4.5
  **+ cosine embedding** sebagai fitur ke-13. Bentuk ini menjaga agar penambahan
  penampilan bisa diukur sebagai delta yang bersih terhadap garis dasar pembanding 0,4282.
...
| **C2** | pengklasifikasi potongan khusus (ConvNeXt, pola Fase 6) dilatih ulang **hanya di 716 pohon train** | ~2 jam GPU + regenerasi citra terpotong (*crop*) (~15 mnt, `REGENERASI.md`) |
| **C3** | pengklasifikasi **multi-tampak**: satu model memproses seluruh pool sekaligus (attention antar tampak), bukan per-sisi lalu digabung | ~4 jam GPU |
...
| **R1** | skor keyakinan (*confidence*) tertinggi (usulan Anda) | rapuh: satu tampak berkeyakinan-tinggi-tapi-salah unggul telak |
...
| **R5** | agregator terlatih (DeepSets/attention) | kapasitas tertinggi, risiko penyesuaian berlebih tertinggi (7.427 tandan train) |
...
3. **Keluarannya kontinu**, jadi hitungan per kelas per pohon bisa dijumlahkan
   sebagai soft count untuk jalur pencacahan hilir tanpa membuang informasi.
...
Tangga R0→R4 dijalankan **dua kali**: sekali dengan tautan model batas atas teoretis (GT) dan
...
- **Dekomposisi model batas atas teoretis vs end-to-end wajib** di setiap tabel. Tanpa itu, hasil
...
|---|---|---|---|
| **G0** nilai pooling | PT-E-001 | Aturan terbaik dengan **tautan oracle** mengalahkan R0 ≥ **+2,0 pp** akurasi per-tandan di val, CI95 bootstrap-pohon tidak mencakup nilai nol | Separuh-kelas dari proposal **mati**. Berhenti; pertahankan hanya perubahan satuan evaluasi (§4.3), yang sudah terbukti tanpa perlu eksperimen apa pun |
| **G1** penaut | PT-E-002 | F1 pasangan val ≥ **0,65** DAN ARI ≥ **0,55**, diuji lebih dulu pada **algoritma dedup yang sudah ada** (§5.2), baru pada embedding re-ID kalau yang pertama belum lolos | Penampilan terlatih pun tidak menyelesaikan penautan → catat sebagai pemalsuan kedua E-007 (kali ini dengan metrik penautan langsung), berhenti di analisis model batas atas teoretis |
| **G2** end-to-end | PT-E-003 | akurasi per-tandan dengan penaut nyata ≥ akurasi model batas atas teoretis − **2,0 pp** | Bottleneck-nya L, bukan A, hasil negatif yang terlokalisasi dan tetap layak dilaporkan |
| **G3** pencacahan | PT-E-004 | macro MAE hitungan pohon < **0,356** (rekor koreksi global k=1,8905, `E-007/report_test.json`) | Konfirmasi ulang E-007: penautan eksplisit tetap berada di bawah pembagian global untuk pencacahan. Opsional, tidak memblokir G0–G2 |
...
- **0,356 (G3)**: angka yang tercatat di `E-007/report_test.json` untuk koreksi
  global; penautan terbaik E-007 waktu itu 1,392, lebih rendah 4×.
...
|---|---|---|---|---|
| **PT-E-001** | Rangka evaluasi per-tandan + plafon model batas atas teoretis. Pemetaan deteksi→tandan, R0–R4 di atas tautan GT, CI bootstrap-pohon. Memakai `pred_sel5_953_rgb_test.npz` + inferensi val | — | **CPU, ~1 jam** | **G0** |
| **PT-E-002a** | **Ukur algoritma dedup yang sudah ada** pada metrik penautan langsung (P/R/F1 pasangan, ARI), garis dasar pembanding sebenarnya, §5.2 | PT-E-001 lolos + kode dedup-nya | CPU, ~1 jam | **G1** |
...
| **PT-E-003** | Tangga aturan keputusan R0–R5 di atas penaut nyata; bandingkan ke model batas atas teoretis | PT-E-002 | CPU (R5: ~1 jam GPU) | **G2** |
| **PT-E-004** | Pencacahan dari jumlah pool vs k=1,8905 vs Ridge+F_all | PT-E-003 | CPU | G3 |
| **PT-E-005** | (opsional) pengklasifikasi multi-tampak C3, bentuk paling setia ke sketsa | PT-E-003 lolos | GPU ~4 jam | — |
...
3. **Pohon 8-sisi hanya 45** (train 34 / val 5 / test 6). Tidak boleh ada
...
| Pengklasifikasi awal | C1 (skor detektor) | cukup untuk melewati G0 tanpa training apa pun |
...
2. **Sampai mana?** Berhenti di kelas per tandan (G0–G2), atau lanjut sampai
   pencacahan per pohon (G3) yang menantang E-007 secara langsung?
