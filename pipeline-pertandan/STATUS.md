...
| ✅ | **PT-E-001** plafon model batas atas teoretis (*oracle*) + gerbang **G0** | **LOLOS** | penggabungan +4,36 pp test, CI95 [+2,33; +6,25] |
...
| ✅ | **PT-E-002b** embedding re-ID (ResNet-18, SupCon) | selesai | fungsi rugi 4,62 → 0,317 (30 epoch, 433 s) |
...
| ✅ | **PT-E-003** ulang dengan penaut sah (E) | **G2 GUGUR** | −2,36 pp vs model batas atas teoretis; tapi penggabungan +4,85 pp CI [+2,03; +7,81] |
| ✅ | **PT-E-004** pencacahan (*counting*): pool vs k-global vs Ridge+F_all | **G3 GUGUR** | 3,3422 vs 1,0542 (Ridge) |
...
| ✅ | **PT-E-012** pengklasifikasi multi-tampak (C3) | **DIPALSUKAN** | C3 −3,06 pp vs C2; C2 −1,21 pp vs C1 |
...
| ✅ | **PT-E-004** pencacahan diulang dengan penaut baru | **G3 tetap GUGUR** | 3,4610 vs 1,0542 (Ridge) |
...
### G0: nilai penggabungan (tautan model batas atas teoretis), dipisah dari rekalibrasi
...
| **G3** pencacahan | **GUGUR** | macro MAE 3,4610 vs 1,0542 (Ridge+F_all) |
...
Yang tidak konklusif bukan karena efeknya hilang, melainkan karena test 352 hanya
95 pool multi-tampak lawan 758 di 953: lebar CI 10,2 pp lawan 3,9 pp. Uji itu
tidak punya daya untuk memutuskan.

Catatan tambahan: R4 berada di bawah R2 di test 352 (0,6946 vs 0,7094) padahal unggul
+8,3 pp di val, `tau` dicari di 52 pohon val, terlalu sedikit, jadi penyesuaian berlebih.
...
2. **Tensor mentah `(1,300,6)` tidak boleh dipakai langsung**, memuat baris
   duplikat, mAP50 menurun drastis ke 0,1342. Pakai `predict()`.
...
6. **Kelas GT dan kelas prediksi tidak boleh dipertukarkan di penaut.**
   ...
   di kotak GT lalu dipakai atas kelas PREDIKSI, dan di sana aturan itu hanya
...
- Lokasi algoritma dedup yang sudah ada. Tanpa itu, garis dasar pembanding (*baseline*) penaut terpaksa
...
Ketiganya tetap dijalankan atas permintaan, dengan kerangka utama (*backbone*)/fungsi rugi berbeda untuk
yang sudah pernah dicoba.

| | Langkah | Status | Hasil |
|---|---|---|---|
| ✅ | **PT-E-014** kerangka utama ConvNeXt untuk modul C | **SEBAGIAN** | C3 −5,41 -> −2,14 pp vs C1; tetap tidak mengalahkan C1 |
| ✅ | **PT-E-015** fungsi rugi ordinal CORAL | **SEBAGIAN** | +2,35 pp C2 (resnet18); arah konsisten, magnitudo dalam derau seed |
...
| ✅ | **PT-E-024** propagasi skor keyakinan (*confidence*) lintas-view | **NAIK** | mAP50 0,5965; mAP50-95 0,2743; macro-F1 operasi 0,5906 |
| ✅ | **PT-E-025** evaluasi end-to-end global 1-ke-1 | **SELESAI** | P/R pool 0,8530/0,8116; acc matched 0,7322; macro-F1 fisik 0,5867 |
| ✅ | **PT-E-026** pencacahan multi-bank | **SELESAI / DIKOREKSI** | macro tetap 1,0039; regresor total compact best observed TEST 1,4882, bukan Σ4 lama 1,7795 |
...
| ✅ | **PT-E-028** pencacahan CatBoost regularized | **SELESAI / TIDAK JADI CHAMPION** | TEST macro 1,0236; class ±1 0,7480; tree ±1 0,3386; kepala total langsung 1,5512 |
...
pool terbaik 2,880 tetap berada di bawah regresor pencacahan sekitar 1,00, sehingga
...
kenaikan sudah ada di garis dasar pembanding, bukan dari intervensi.
...
**0,7464** pada protokol yang sama. Setiap anggota C2 Berada di bawah C1 (0,682-0,706
lawan 0,7208) tetapi gabungannya unggul, jadi yang PT-E-012 tutup adalah jalur
...
**PT-E-034 (plafon) menjawab apakah 0,80 masih mungkin.** Model batas atas teoretis pilih-anggota
= **0,8739** (informasinya ADA), tetapi rata-rata berbobot dengan bobot dipas
LANGSUNG di TEST mencapai batas saturasi **0,7523** -- hasil sesuai protokol 0,7439 hanya 0,84 pp di bawahnya.
...
2. **Prediksi out-of-fold di gabungan 641 train + 86 val** (K-fold, tiap anggota
   ...
   satunya yang membuat perbaikan berikutnya bertahan ke TEST.
...
| model batas atas teoretis di wilayah berselisih | 0,9741 |
| korelasi keyakinan vs benar | **+0,1185** |

Untuk 0,80 total, wilayah berselisih harus naik ke **0,765** (+15,3 pp). Sinyalnya
ADA (model batas atas teoretis 0,9741) tetapi **tidak terbaca dari keyakinan anggota** -- memilih
anggota paling yakin (0,5711) bahkan berada di bawah merata-ratakan (0,6121) dan hanya
