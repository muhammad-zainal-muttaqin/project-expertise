...
| pencacahan (*counting*) | jumlah tandan per kelas B1–B4 di tingkat kohort (sasaran utama BBC) | per kohort (validation set atau test set) | bias per kelas dan macro-avg \|bias\|; MAE cacah total per pohon |
...
| Plafon lokalisasi sempurna (*oracle*) + ConvNeXt-Tiny / Ridge | — | — | 0,6569 | — | — | — | **1,058** | model batas atas teoretis (*oracle*) | `AF-E-005` |
...
### Pencacahan kohort per kelas (sasaran BBC)
...
|---|---|---|
| Ambiguitas Makro-*F1* (`AF-E-012`) | Terdapat dua nilai: $0,6692$ (klaster terpasangkan) dan $0,5201$ (ujung ke ujung). Hanya nilai $0,5201$ yang setara dengan garis dasar pembanding (*baseline*) $0,6034$. | [`EVIDENCE.md`](../docs/research_2026-09-06/EVIDENCE.md) |
...
| Sel `avg \|bias\|` tak terisi (2 sel: `V2-E-046`, `V2-E-047`) | Formula (makro-rerata \|bias relatif\| B1–B4) diverifikasi persis terhadap keempat nilai `Wave-V2` yang sudah terisi. `V2-E-045` (4 sel), `V2-E-043` (2 sel), dan `AF-E-012` (2 sel) berhasil diisi sesi ini karena artefak sumbernya (*dump* proposal box, atau bobot detektor+*classifier*) ternyata tersimpan di `results/` Git atau bucket cadangan. `V2-E-046`/`V2-E-047` diinvestigasi mendalam (sesi 2026-09-07) sampai akar penyebabnya, bukan sekadar dicoba sekali: **`V2-E-046`** (953 val, *stacking* DINOv2-Large), **direkonstruksi penuh sesi ini, tetapi tidak mencapai angka terpublikasi, sehingga sel TETAP `·`.** *Pipeline* fitur bertingkatnya tidak pernah tersimpan (indeks *crop*, fitur DINOv2), termasuk skrip **pembangun indeks *crop*-nya sendiri** yang tidak ada di manapun (hanya skrip pembacanya yang tersimpan), keduanya berhasil direkayasa ulang dari kode konsumen (skema *crop* diturunkan dari `extract_aux_crops.py`, proposal box dari kerja `V2-E-045` sesi ini). `large_stacker.py` ternyata bukan satu model DINOv2-Large saja, melainkan ansambel **empat pakar** (DINOv2-Large `large_member_head.py`, DINOv2-Base `member_head.py`, *multiscale* dua skala tambahan `multiscale_member_head.py`, plus vote detektor) digabung lewat pencarian *grid* berbobot 1.200 kombinasi. Seluruh empat cabang berhasil dibangun ulang sesi ini (tiga sesi ekstraksi DINOv2 terpisah: *Large* pada citra terpotong (*crop*) konteks 1,5×, *Base* pada citra terpotong konteks 1,5×/1,0×/2,0×) dan digabung persis mengikuti algoritma `large_stacker.py`. Hasil rekonstruksi penuh: akurasi kelas tertaut = **71,64%**, F1 fisik identik dengan nilai terpublikasi (0,8060, menegaskan penautan/deteksi tereplikasi benar), tetapi 5,2 poin persentase di bawah akurasi kelas yang sudah terpublikasi pada baris ini (76,8%). Titik optimum pencarian *grid* memberi bobot mendekati nol pada cabang DINOv2-Base dan *multiscale* (`base_w=0`, `log_w=0`, `ms_w=0,05`), berarti kedua cabang itu nyaris tidak berkontribusi pada rekonstruksi ini, berbeda dari eksperimen asli yang menyimpulkan kombinasi keempatnya bermanfaat. Akar selisihnya tidak teridentifikasi meski *pipeline* lengkap sudah dibangun ulang (kandidat: detail algoritma *crop* yang sedikit berbeda, versi *checkpoint* DINOv2 yang berbeda, atau varian implementasi lain yang tidak terekam di kode yang tersisa). Mengisi `avg \|bias\|` dari rekonstruksi 71,64% ini akan tidak konsisten dengan kolom `classification` (76,8%) pada baris yang sama, lebih baik `·` yang sesuai bukti daripada angka yang terlihat koheren tapi sebenarnya tidak. **`V2-E-047`** (763-depth val, komposisi lintas-lapis) jauh lebih terhambat: rantai impornya (`composition_aware_head.py` → `pipeline_v2.py` → `harness.py`) butuh `link_global_setpartition.py`, solver **MILP *global set-partition*** untuk penautan proposal 763-depth, dan modul ini **tidak ada sama sekali** di repositori maupun bucket cadangan (dicari menyeluruh). Ini bukan berkas data yang bisa diregenerasi ulang, melainkan kode algoritma optimisasi itu sendiri; merekonstruksinya berarti menulis ulang solver MILP dari nol tanpa rujukan formulasi asli, kategori pekerjaan yang jauh lebih besar dan berisiko dibanding seluruh sel lain yang berhasil diisi sesi ini, sehingga **tidak dikerjakan**. Baris model batas atas teoretis `AF-E-005` dikoreksi dari `·` menjadi `—`: studi ini murni plafon klasifikasi per-*crop*, tanpa tahapan penautan/pencacahan tingkat pohon, sejalan dengan kolom `dedup`/`classification` yang sudah `—`. `V2-E-043` (kedua korpus, uji): medan `confusion_matrix` ditambahkan ke `multiview_metrics()` (`scripts/eval_remote_pipeline_postprocess.py`), F1 dan total acuan tervalidasi presisi penuh terhadap nilai `dedup` terpublikasi (0,8296 dan 0,8590). `AF-E-012` (kedua split): bobot ditemukan di bucket (`audit_forensik_2026-09-06/`), pipeline diregenerasi penuh, F1/`class4_acc` cocok dalam <0,005. `V2-E-045` (4 sel): medan `confusion_matrix` ditambahkan ke `evaluate_payload()` (`scripts/evaluate_remote_count_reconciled.py`), F1/`class4_acc` hasil rekonstruksi cocok dalam <0,004 dari nilai terpublikasi di keempat sel (satu cocok presisi penuh). | Audit sesi 2026-09-07 |
...
| Pencacahan kohort per kelas | Diturunkan dari medan `metrics.classification.confusion_prediction_rows` pada artefak `Wave-V2` |
