...
| YOLO26m + penaut terlatih + Ridge (Pipeline Panen) | · | — | 0,7586 | 71,7% | 25,65% | 1,374 | val | `AF-E-012` |
...
| Sel `detection` tak terisi, RF-DETR-L (`V2-E-001`, tabel 953) | Bobot `runs/rfdetr_l_e60_i1280_v2repro/` tidak ada di repositori maupun bucket cadangan Hugging Face; tidak ada dump prediksi tersimpan. Tidak dapat diisi tanpa pelatihan ulang 60 *epoch* penuh — belum dikerjakan sesi ini. | Audit sesi 2026-09-07 |
...
| Sel `detection` tak terisi, Pipeline Panen (`AF-E-012`, val, tabel 953) | **Koreksi:** klaim sebelumnya bahwa bobot detektor (`runs_panen/agnostik_m1280/`) dan classifier (`crops953/corn_best.pt`) tidak ada di bucket cadangan adalah **keliru** — keduanya ditemukan sesi ini di bawah prefix `audit_forensik_2026-09-06/`, bukan prefix top-level yang dicari sebelumnya. Pipeline diregenerasi penuh (lihat entri `avg \|bias\|` di bawah), tetapi sel `detection` (val) tetap `·`: satu-satunya dump deteksi mentah tersedia (`dets.pkl`) dibangun dengan ambang `conf=0,10` (bawaan `detect()`), tidak sebanding dengan standar `conf=0,001` di seluruh kolom `detection` lain — dikonfirmasi lewat AP50 dari dump ini pada test (0,7487) vs. nilai `detection` AF-E-012/AF-E-011 yang sudah terpublikasi (0,8104, detektor identik), selisih 0,062 terlalu besar untuk derau pelatihan ulang biasa. Perlu satu inferensi GPU tambahan `conf=0,001` — ditunda, GPU sedang dipakai retrain RF-DETR-L. Kolom `det+class` pada baris ini tetap `—` karena detektornya class-agnostic murni (setara baris uji), bukan sekadar belum dievaluasi. | [`class_bias_afe012_953_2026-09-07.json`](../results/class_bias_afe012_953_2026-09-07.json) |
| Sel `avg \|bias\|` tak terisi (6 sel: `V2-E-045` ×4, `V2-E-046`, `V2-E-047`) | Formula (makro-rerata \|bias relatif\| B1–B4) diverifikasi persis terhadap keempat nilai `Wave-V2` yang sudah terisi (dari medan `gsp_best_by_class.metrics.classification.confusion_prediction_rows`). Diagnosis presisi per node (sesi 2026-09-07): **`V2-E-045`** (4 sel) memerlukan inferensi GPU pada partisi *train* (model *count* dilatih di sana) — belum dikerjakan, GPU dipakai retrain `V2-E-001`/`V2-E-001` RF-DETR-L. **`V2-E-046`** (953 val): `harness.py` menghitung `confusion_prediction_rows` secara internal tetapi `large_stacker.py` membuangnya sebelum disimpan ke disk; regenerasi butuh fitur DINOv2-Large yang sudah diekstrak dan model *stacker* `.joblib` — keduanya dikonfirmasi tidak ada, baik di kontainer sesi ini **maupun di bucket cadangan** (diperiksa langsung). **`V2-E-047`** (763-depth val): pola sama, `composition_aware_head.py`; cache fitur dan model `.joblib`-nya juga tidak ada di bucket. **Pola umum:** `V2-E-043` dan `AF-E-012` berhasil diisi karena artefak sumbernya (dump proposal box, atau bobot detektor+*classifier*) ternyata tersimpan di `results/` Git atau bucket cadangan (lihat entri `AF-E-012` di atas — pencarian awal sesi ini sempat keliru menyimpulkan bobotnya hilang). `V2-E-046`/`V2-E-047` benar-benar berbeda: cache fitur dan model perantaranya memang tidak pernah dibackup sama sekali, bukan sekadar salah lokasi pencarian. Baris oracle `AF-E-005` dikoreksi dari `·` menjadi `—`: studi ini murni plafon klasifikasi per-*crop*, tanpa tahapan penautan/pencacahan tingkat pohon, sejalan dengan kolom `dedup`/`classification` yang sudah `—`. `V2-E-043` (kedua korpus, uji) terisi sesi ini: medan `confusion_matrix` ditambahkan ke `multiview_metrics()` (`scripts/eval_remote_pipeline_postprocess.py`), dijalankan pada proposal WBF uji yang sudah tersimpan, hasil F1 dan total acuan tervalidasi presisi penuh terhadap nilai `dedup` yang sudah dipublikasikan (0,8296 dan 0,8590). `AF-E-012` (kedua split) terisi sesi ini setelah bobot ditemukan di bucket (`audit_forensik_2026-09-06/`); pipeline diregenerasi penuh, F1/`class4_acc` hasil rekonstruksi cocok dalam <0,005 dari nilai terpublikasi. | Audit sesi 2026-09-07 |
