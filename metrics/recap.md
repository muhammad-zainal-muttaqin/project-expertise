...
Metrik utama: $mAP50$ sadar-kelas (*class-aware*); metrik pendamping: $mAP50\text{--}95$ serta $AP50$ untuk setiap kelas kematangan (B1–B4). Seluruh evaluasi menggunakan protokol baku `pycocotools.COCOeval` pada partisi data uji. Tabel diurutkan berdasarkan nilai $mAP50$ tertinggi.

| Konfigurasi Sistem | Korpus Data | $mAP50$ | $mAP50\text{--}95$ | $AP_{B1}$ | $AP_{B2}$ | $AP_{B3}$ | $AP_{B4}$ | ID Simpul | Status Evaluasi |
|---|---|---:|---:|---:|---:|---:|---:|---|---|
| RF-DETR-L, bank `combined1716` | 763-depth | **0,6711** | 0,2748 | 0,8044 | **0,7187** | **0,7373** | 0,4239 | `V2-E-042` | Data Uji |
| WBF [YOLO26l+RT-DETR-L+RF-DETR-L], bank `combined1716` | 763-depth | 0,6691 | **0,2757** | 0,8280 | 0,6919 | 0,7349 | 0,4214 | `V2-E-042` | Data Uji |
| Plafon lokalisasi sempurna + ConvNeXt-Tiny | 953 | 0,6569 | N/A¹ | 0,7653 | 0,4912 | 0,7625 | **0,6088** | `AF-E-005` | Batas Atas (*Oracle*) |
| WBF + *re-ranker* | 763-depth | 0,6552 | 0,2701 | **0,8288** | 0,6863 | 0,7362 | 0,3693 | `MAP_BOOST` | Data Uji |
| RT-DETR-L, bank `combined1716` | 763-depth | 0,6309 | 0,2496 | 0,7865 | 0,6652 | 0,7160 | 0,3559 | `V2-E-042` | Data Uji |
| RF-DETR-L native | 763-depth | 0,6129 | 0,2335 | 0,7758 | 0,6353 | 0,6997 | 0,3406 | `V2-E-034` | Data Uji |
| RF-DETR-L native | 953 | 0,6012 | 0,2747 | 0,8150 | 0,5184 | 0,6553 | 0,4160 | `V2-E-001` | Data Uji |
| WBF + *re-ranker* | 953 | 0,5970 | 0,2743 | 0,8042 | 0,4942 | 0,6566 | 0,4328 | `MAP_BOOST` | Data Uji |
| RF-DETR-L native | `combined1716` | 0,5960 | 0,2522 | 0,7654 | 0,5394 | 0,6652 | 0,4141 | `V2-E-035` | Data Uji |
| WBF, bank `combined1716` | 953 | 0,5861 | 0,2753 | 0,7869 | 0,4866 | 0,6513 | 0,4197 | `V2-E-042` | Data Uji |
| RT-DETR-L native | 953 | 0,5781 | 0,2629 | 0,7874 | 0,4614 | 0,6371 | 0,4266 | `V2-E-001` | Data Uji |
| YOLO26l, bank `combined1716` | 763-depth | 0,5765 | 0,2387 | 0,7839 | 0,6088 | 0,6380 | 0,2754 | `V2-E-042` | Data Uji |
| RT-DETR-L native | `combined1716` | 0,5745 | 0,2458 | 0,7308 | 0,5120 | 0,6465 | 0,4089 | `V2-E-035` | Data Uji |
| RT-DETR-L native | 763-depth | 0,5580 | 0,2055 | 0,7377 | 0,5889 | 0,6607 | 0,2445 | `V2-E-034` | Data Uji |
| WBF native | `combined1716` | 0,5538 | N/A² | 0,7286 | 0,4732 | 0,6372 | 0,3760 | `V2-E-039` | Data Uji |
| YOLO26l native | 953 | 0,5435 | 0,2564 | 0,7705 | 0,4479 | 0,6050 | 0,3506 | `V2-E-001` | Data Uji |
| YOLO26s 960 px, replikasi audit | 953 | 0,5433 | N/A² | 0,7588 | 0,4495 | 0,6095 | 0,3553 | `AF-E-006` | Data Uji |
| YOLO26l native | `combined1716` | 0,5389 | 0,2395 | 0,7298 | 0,4766 | 0,6075 | 0,3419 | `V2-E-035` | Data Uji |
| YOLO26l native | 763-depth | 0,5163 | 0,1906 | 0,6847 | 0,5877 | 0,6005 | 0,1920 | `V2-E-034` | Data Uji |
...
Metrik utama: $AP50_{agn}$ dengan seluruh kelas kematangan dilebur menjadi satu kategori objek tunggal (*foreground/background*); metrik pendamping: $AP50\text{--}95_{agn}$, Presisi, Daya Tangkap (*Recall*), dan *F1* pada titik operasi ambang keyakinan $0,25$ dan ambang $IoU = 0,5$. Tabel diurutkan berdasarkan nilai $AP50_{agn}$ tertinggi.

| Konfigurasi Sistem | Korpus Data | $AP50_{agn}$ | $AP50\text{--}95_{agn}$ | Presisi | Daya Tangkap | *F1* | Jumlah Citra | ID Simpul |
|---|---|---:|---:|---:|---:|---:|---:|---|
...
| YOLO26l, kelas dilebur | 953 | 0,7388 | 0,3312 | 0,7547 | 0,6336 | 0,6889 | 588 | `V2-E-017` |
...
Metrik utama: *F1 fisik* pada ambang pencocokan spasial $IoU \ge 0,5$; metrik pendamping: Presisi fisik dan Daya tangkap fisik (*recall*) pada tingkat klaster pohon. Tabel diurutkan berdasarkan nilai *F1 fisik* tertinggi.

| Konfigurasi Sistem | Korpus Data | Partisi Evaluasi | $n$ Pohon | *F1 Fisik* | Presisi Fisik | Daya Tangkap Fisik | ID Simpul |
|---|---|---|---:|---:|---:|---:|---|
...
> [!NOTE]
> Evaluasi inferensial menunjukkan bahwa selang kepercayaan 95% dari kedua profil utama saling bertumpang tindih (lihat Bagian 7.1). Dengan demikian, selisih performa antara $0,8534$ dan $0,8387$ belum signifikan secara statistik.

---

## 6. Klasifikasi Kematangan

Metrik utama: Akurasi *matched-class* pada klaster yang terasosiasi secara tepat dengan data acuan; metrik pendamping: Makro-*F1* ujung ke ujung (*end-to-end*) yang turut memperhitungkan penalti klaster palsu (*false positive*) dan tandan terlewat (*false negative*), serta nilai *F1* per kelas kematangan. Tabel diurutkan berdasarkan akurasi tertinggi.

| Konfigurasi Sistem | Korpus Data | Partisi Evaluasi | Akurasi *Matched-Class* | Makro-*F1* E2E | $F1_{B1}$ | $F1_{B2}$ | $F1_{B3}$ | $F1_{B4}$ | ID Simpul |
|---|---|---|---:|---:|---:|---:|---:|---:|---|
...
Metrik utama: *Mean Absolute Error* (MAE) cacah tandan total per pohon; metrik pendamping: Akurasi toleransi $\pm 1$ tandan dan akurasi pencacahan tepat persis (selisih nol). Tabel diurutkan berdasarkan nilai MAE terendah (terbaik).

| Konfigurasi Sistem | Korpus Data | Partisi Evaluasi | $n$ Pohon | MAE Total | Akurasi $\pm 1$ | Akurasi Tepat Persis | ID Simpul |
|---|---|---|---:|---:|---:|---:|---|
...
⁶ Lapisan modul regresi Ridge beroperasi langsung pada vektor fitur proposal terlepas dari modul penaut, sehingga kedua profil menghasilkan nilai MAE yang identik pada partisi validasi 953.
...
### 7.1 Estimasi Selang Kepercayaan 95% Profil Terkunci Uji
...
| $mAP50$ klasifikasi biner (siap panen vs belum) $\ge 0,85$ | 0,7754 | 0,8766 | Terjangkau secara empiris |
| Pencacahan total per pohon tepat persis $\ge 50\%$ | 0,2741 | 0,2900 | **Tidak terjangkau secara teoretis** |
| Pencacahan siap panen per pohon (toleransi $\pm 1 \ge 95\%$) | 0,957–0,965 | 1,000 | **Tercapai** |

### 9.2 Simulasi Kebutuhan Akurasi Kematangan (`AF-E-005`)

Simulasi proyeksi untuk mengetahui tingkat akurasi klasifikasi kematangan per citra terpotong (*crop*) yang dibutuhkan guna menaikkan $mAP50$ deteksi 4 kelas:

| Akurasi Klasifikasi Kematangan per Citra Terpotong | Proyeksi $mAP50$ Deteksi Empat Kelas |
|---:|---:|
| $0,661$ (Capaian model saat ini) | $0,587$ |
| $0,800$ | $0,735$ |
| $0,900$ | **$0,847$** |
| $0,950$ | $0,927$ |

> [!WARNING]
> Target rekayasa $mAP50 \ge 0,85$ menuntut akurasi modul pengklasifikasi kematangan minimal $90\%$, yaitu sekitar 24 poin persentase di atas kapabilitas tertinggi model yang berhasil dicapai pada penelitian ini ($66,35\%$).
...
Makro-MAE *oracle* tercatat sebesar $0,312$, konsisten dengan estimasi batas atas historis $0,275$–$0,277$ pada [`docs/REKAP.md`](../docs/REKAP.md) §2. Rata-rata faktor multiplisitas kemunculan per tandan tercatat sebesar $k = 1,905$ dengan simpangan baku $0,384$.
...
|---|---|---|
| Ambiguitas pelaporan Makro-*F1* pada `AF-E-012` | Terdapat dua nilai valid: $0,6692$ (khusus klaster yang terpasangkan) dan $0,5201$ (ujung ke ujung). Hanya nilai $0,5201$ yang setara dan adil untuk diperbandingkan dengan baseline $0,6034$. | [`EVIDENCE.md`](../docs/research_2026-09-06/EVIDENCE.md) |
| Konfigurasi khusus Pipeline Panen (`AF-E-012`, `AF-E-013`) | Menggunakan 132 pohon, detektor tunggal YOLO26m, dan modul penaut audit mandiri. Tidak setara secara langsung dengan profil terkunci, namun unggul pada estimasi B1 toleransi $\pm 1$ ($0,970$), akurasi ordinal $\pm 1$ ($0,9946$), dan akurasi biner ($0,8678$). | `AF-E-012`, `AF-E-013` |
| Klarifikasi pelanggaran kendala fisik multi-tampak (`AF-E-010`) | Temuan awal bahwa $45,3\%$ klaster melanggar kendala fisik telah dikoreksi oleh `AF-E-014`: pelanggaran hanya terjadi pada mode pencarian heuristik tanpa batas. Pada profil terkunci dengan `max_size` $\le 3$, tingkat pelanggaran adalah $0,00\%$ (0 dari 630 konfigurasi berubah). Pada `AF-E-016`, metode *Anchor A* (`max_size` = 4) juga terbukti aman (0 dari 135 pohon terpengaruh). | `AF-E-014`, `AF-E-016` |
| Status rangkaian eksperimen `AF-E` | Seluruh eksperimen audit forensik (`AF-E`) menggunakan detektor berkapasitas lebih kecil, modul penaut eksperimental, atau inversi taksonomi. Angka-angka ini berfungsi sebagai instrumen diagnostik pelengkap dan tidak menggantikan angka acuan profil terkunci. | [Atlas 07](07_audit_forensik.md) |
| Potensi kebocoran partisi data (*data leakage*) pada `combined1716` | Irisan identitas pohon (`tree_id`) antara partisi latih `combined1716` dan partisi uji lokal belum diaudit secara tuntas, sehingga risiko kontaminasi informasi belum terkuantifikasi. | [`ANALISIS_PIPELINE_MENDALAM.md`](../docs/ANALISIS_PIPELINE_MENDALAM.md) |
| Penggunaan partisi uji pada iterasi historis | Angka partisi uji berfungsi sebagai konfirmasi atas profil yang telah dikunci pada partisi validasi (`V2-E-045`), bukan sebagai partisi terisolasi mutlak yang belum pernah tersentuh. | `V2-E-045` |
| Ketergantungan plafon performa pada model pengklasifikasi | Batas atas $mAP50 = 0,6569$ dievaluasi berdasarkan model pengklasifikasi dengan akurasi $66,35\%$ per citra terpotong (merefleksikan kapabilitas model dalam rentang $0,62$–$0,70$), bukan batas teoretis mutlak teori informasi. | `AF-E-005` |
...
|---|---|
| Tabel 3 (Deteksi Empat Kelas) | [`combined1716`](../results/combined1716), [`new763`](../results/new763), medan `splits.test.mAP50`; [`perkelas_pycoco_v2repro.json`](../results/perkelas_pycoco_v2repro.json); [`extra_metrics_sesi2026-08.json`](../results/extra_metrics_sesi2026-08.json); [`map_boost_artifacts`](../results/remote_eval_2026-08-28/map_boost_artifacts), medan `test_metrics.classaware`; [`detector_matrix.json`](../results/audit_forensik_2026-09-06/detector_matrix.json) |
| Tabel 4 (Deteksi Agnostik) | [`class_agnostic_metrics_audit_2026-09-03.json`](../results/class_agnostic_metrics_audit_2026-09-03.json), medan `rows[]`; [`map_boost_artifacts`](../results/remote_eval_2026-08-28/map_boost_artifacts), medan `test_metrics.agnostic` |
| Tabel 5, 6, 7 (Profil `V2-E-045`) | [`pipeline_combined1716_generalization_locked.json`](../results/remote_eval_2026-08-27/metrics/pipeline_combined1716_generalization_locked.json) |
| Tabel 5, 6, 7 (Profil `Wave-V2` & CI95) | [`gsp_artifacts/953`](../results/remote_eval_2026-08-28/gsp_artifacts/953/results_test_locked.json), [`gsp_artifacts/depth`](../results/remote_eval_2026-08-28/gsp_artifacts/depth/results_test_locked.json), [`anchor_check.json`](../results/remote_eval_2026-08-28/validation_wave/reports/anchor_check.json) |
| Tabel 5, 6, 7 (Pipeline Panen) | [`panen/panen_final.json`](../results/audit_forensik_2026-09-06/panen/panen_final.json) |
| Tabel 8 (Bias per Kelas) | Diturunkan dari medan `metrics.classification.confusion_prediction_rows` pada artefak `Wave-V2` |
| Tabel 9 (Plafon & Target Rekayasa) | [`experiments/AUDIT-FORENSIK-2026-09-06.md`](../experiments/AUDIT-FORENSIK-2026-09-06.md), [`docs/AUDIT-FORENSIK-2026-09-06.md`](../docs/AUDIT-FORENSIK-2026-09-06.md) |
